...
The results show how the attack was utterly ineffective even though the network was still being built at the time the Sybils started to join. The average percentage of attacinkg neighbours (Fig.~\ref{fig:ex-atk-neigh}) is significantly low.

Despite that, there is no gap that big in the percentage of Sybils in the peer cache (Fig.~\ref{fig:ex-atk-known}) between the two attack scenarios, thus proving the attackers could disseminate well their addresses.

This leads to two major conclusions: DNS and time at which the information is began to be spread.
...
