...
Also DNS bootstrap peer lists played their role to determine the efficacy of the attack. In Fig.~\ref{fig:dns} the reader can see that in many cases the concentration of malicious addresses is high, thus increasing the chance of a node on fresh bootstrap connecting to a malicious node first.  
...
In this scenario DNS peer lists did not have any attacker, since they cannot contain any node on fresh bootstrap.
...
The results in Fig.~\ref{fig:ext-cov} show how the attack was utterly ineffective even though the network was still being built at the time the Sybils started to join. The average percentage of attacking neighbours (Fig.~\ref{fig:ex-atk-neigh}) is significantly lower.

Despite that, there is no gap that big in the percentage of Sybils in the peer cache (Fig.~\ref{fig:ex-atk-known}) between the two attack scenarios, thus stating the attackers could still disseminate well their addresses.
...
Although the first factor has been surely influential, it is required to assess the importance of the latter.

The data in Fig.~\ref{fig:beginning} shows the percentage of malicious addresses in peer caches before the bootstrapping nodes start to join.
