...
Bitcoin is a cryptocurrency~\cite{nakamoto}
...
In a block as set of transactions is bundled together and stored as a Merkle Tree, which ensures the data is unaltered and undamaged from the moment it is put into the block.

On top of that each block contains the hash of the previous block in the blockchain and its own hash, thus granting that blocks cannot be tempered, for altering a transaction would result in mismatching hashes in the Merkle Tree, the stored block hash and in the hash in the next block in the blockchain.
...
Looking for the right nonce implies scanning and is therefore exponential on $n$ number of zeroes. Validating a block is instead a fast, one-step task, i.e. hash of the block.
...
In particular, it is in the interest of miners to spread as fast as possible newly mined blocks in order to claim rewards and not to waste time on stale views of the blockchain.
...
\texttt{inv} messages are also gossiped when a node receives a new block. The described message exchange can be found in Fig.~\ref{fig:synch}.
...
whenever the block chain goes into an incosistent state all nodes update their local copy with the state on which the majority of nodes agree -  thus teh correct state os the ledger is obtained by election

fault-tolerant consensus protocl to ensure that all nodes agree on the order in which entries have been appended to the blockchain.\\
...
As mining is defined as a distributed process and due to the lack of any centralized ledger, nodes may share inconsistent views of the blockchain. This is the case if relayed blocks are not delivered to some nodes or two or more nodes mine a new block approximately at the same time.
...
A fault-tolerant distributed consensus protocol ensures that all nodes agree on the order of the entries of the blockchain. More specifically the protocol states that:
\begin{enumerate}
	...
	\item no coinbase transactions were spent within 100 blocks of their creation
	\item no transactions spend inputs with a \texttt{time\_lock} before the block in which they are confirmed
\end{enumerate} 
 
The PoW based consensus algorithm is generally considered robust and extremely scalable, as no authorization is required to join the network and mine.
