...
The desired property  for distributed data storages is \emph{data consistency}, which implies synchronization and guarantees that the result of reads and writes will be predictable and the same for all peers.

Cryptocurrencies adopt \emph{eventual data consistency}, a relaxed definition of data consistency, which grants that if the ledger is not updated for a time long enough, all peers eventually agree on the state of the memory.

To grant data consistency, distributed architectures need a \emph{consensus mechanism} so that peers would agree on the current memory state.

Nevertheless, achieving consensus is not trivial, since nodes communicate through an unreliable network.

Furthermore, no assumption can be made on the activity of nodes. Peer can undergo failures, be unreachable or, in the worst case scenario, act maliciously. In the latter case, nodes could even try to alter the content of database.

The set of failures to which the system is exposed when no assumptions on the network and on the behaviour of peers can be made are called \emph{Byzantine failures}. The name is derived from the famous Byzantine Generals' Problem~\cite{bgp}.

The problem states that:....

Different protocols and algorithms have been introduced in the past to achieve consensus in case of Byzantine failures, like RAFT and Paxos.
...
The blockchain is a public tamperproof append-only list where all the transactions are stored.
